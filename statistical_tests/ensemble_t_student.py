...
file = "../experiment_results/ensemble_results.npy"

headers = ["Gaussian", "DecisionTree", "Random Stumps"]
...
writer.writerow(row)
for i in range(res.shape[0]):
    row = [f"dataset {i}", f"{mean_scores[i][0]} ({std_scores[i][0]})", f"{mean_scores[i][1]} ({std_scores[i][1]})", f"{mean_scores[i][2]} ({std_scores[i][2]})"]
    writer.writerow(row)
    better_row = []
    better_row.append("better than")
